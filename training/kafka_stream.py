...
class KafkaStream(Stream):
    ...
    def __init__(self, topic, bootstrap_servers, target_idx=-1, n_targets=1, cat_features=None):
        ...
        # self.classes = np.array([[] * self._n_targets], int) if self.n_targets > 1 else np.array([], int)
    ...
    def _find_n_samples(self):
        ...
        """
        partitions = [TopicPartition(self.topic, p) for p in 
        ...
        try:
            ...
            for message in self.consumer:
                ...
                if any(sample.dtypes == 'object'):
                    print(f'Streamed sample contains text or malformatted data. Dropping sample: {self.sample_idx - i}')
            ...
            if (self.target_idx + self.n_targets) == cols or (self.target_idx + self.n_targets) == 0:
                ...
                self.target_names = sample.iloc[:, self.target_idx:].columns.values.tolist()
            else:
                # Take only n_targets columns to the right of target_idx, use the rest as features
                self.current_sample_y = sample.iloc[:, self.target_idx:self.target_idx + self.n_targets].values
                self.target_names = labels[self.target_idx:self.target_idx + self.n_targets]

            self.current_sample_x = sample.drop(self.target_names, axis=1).values
            self.feature_names = sample.drop(self.target_names, axis=1).columns.values.tolist()
            ...
            if np.issubdtype(self.current_sample_y.dtype, np.integer):
                ...
                self.classes = np.unique(np.concatenate((self.classes, sample_classes)))
